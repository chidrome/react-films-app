...
class FilmListing extends Component {
    ...
    handleFaveToggle = (film) => {
        ...
        const filmIndex = this.state.faves.indexOf(film);
            if(filmIndex === -1) {
                // doesn't exist in favorites list yet
        ...
        return (
            <div className="film-list">
                ...
                <div className="film-list-filters">
                    <div className={`film-list-filter ${this.state.filter==="all" ? "is-active" : ""}`} onClick={()=>this.handleFilterClick("all")}>ALL
                        <span className="section-count">{this.props.films.length}</span>
                    </div>
                </div>
                <div className={`film-list-filter ${this.state.filter==="faves" ? "is-active" : ""}`} onClick={()=>this.handleFilterClick("faves")}>Faves
